...
// RoundedRaff Home menu uses one monochrome graphic per item: icon + Vietnamese label.
// HomeActivity still owns the dynamic 4/5/6-item menu model; this renderer only lays out graphics.
void drawHomeMenu(GfxRenderer& r, const HomeRenderContext& h) {
  ...
  auto menuIcon = [](UIIcon icon) -> const uint8_t* {
    switch (icon) {
  ...
  // Icon-only toolbar. Use the native 32x32 monochrome icon assets directly.
  // Do not scale or rotate them here: GfxRenderer::drawIcon() consumes these
  // assets at their native size and applies the X4 bitmap orientation itself.
  ...
  for (int i = 0; i < h.menuCount; ++i) {
    ...
    const uint8_t* icon = menuIcon(h.rowIcon(i));
    if (icon) {
